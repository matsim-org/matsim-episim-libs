...
{       nAbDelta <- input[1]
        ...
        for(i in 1:length(deltaFromPapers$Days)){ 
          ...
          if(deltaFromPapers$Days[i] > boosterDay){
            ...
            nAbOnDay <- nAbOnDay241 * exp(-decayConstant*deltaFromPapers$Days[i])
          }
        ...
        for(i in 1:length(omicronFromPapers$Days)){
          ...
          if(omicronFromPapers$Days[i] > boosterDay){
            ...
            nAbOnDay241 <- nAbOnDay240*refreshFactor
            nAbOnDay <- nAbOnDay241 * exp(-decayConstant*omicronFromPapers$Days[i])
          }
            immunityFactor <- 1 / (1 + nAbOnDay^beta)
            probaWVacc <- 1 - exp(-fact*immunityFactor)
            probaWoVacc <- 1-exp(-fact)
...
optimalParameters <- optim(c(1,1,1,10), calcMSE) 
initialDelta <- optimalParameters$par[1]
initialOmicron <- optimalParameters$par[2]
optimalBeta <- optimalParameters$par[3]
refreshFactor <- optimalParameters$par[4]
...
modelVE <- function(initialNAb, refreshFactor, beta, day){
  ...
  nAbOnDay241 <- nAbOnDay240*refreshFactor
  nAbOnDay <- nAbOnDay241 * exp(-decayConstant*day)
  }
  immunityFactor <- 1 / (1 + nAbOnDay^beta)
  probaWVacc <- 1 - exp(-fact*immunityFactor)
  probaWoVacc <- 1-exp(-fact)
...
ggplot() +
  ...
  scale_colour_manual("", 
                      breaks = c("Nordstroem, Delta", "UKHSA, Delta", "Andrews, Delta", "UKHSA, Delta Booster", "UKHSA, Omicron", "Chemaitelly, Omicron", "Andrews, Omicron", "UKHSA, Omicron booster", "model Delta", "model Omicron"),
                      values = c("navy","dodgerblue2","dodgerblue3", "lightblue","red", "coral", "tomato4", "orange", "dodgerblue4", "darkred"))
