...
rki_and_episim <- episim_final %>%
  ...
  pivot_longer(c(rki_week, rki_old_week, episim_week), names_to = "data_source", values_to = "cases")
...
ggplot(rki_and_episim %>% filter(district == "Mitte"), aes(x = week_year, y = cases)) +
