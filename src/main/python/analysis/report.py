...
df = holiday[(holiday.furtherMeasuresOnOct1=="yes") & (holiday.tracingCapacity > 100)]

fig, ax = plt.subplots(dpi=250, figsize=(7.5, 3.8))
hue = sns.color_palette(n_colors=5)

rki.plot.scatter(x="date", y=["cases"], label=["RKI Cases"], color=palette[5], ax=ax, logy=True)

sns.lineplot(x="date", y="cases", estimator="mean", ci="q95", ax=ax,
             hue="infectionsPerDay", palette=hue, 
             data=df)
...
plt.ylim(bottom=10)
plt.xlim(datetime.fromisoformat("2020-06-01"), datetime.fromisoformat("2021-01-31"))
plt.title("Daily new infections aggregated over all random seeds")