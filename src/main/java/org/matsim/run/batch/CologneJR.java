...
public class CologneJR implements BatchRun<CologneJR.Params> {
	...
	Map<LocalDate, Integer> infPerDayB117 = new HashMap<>();
		...
		infPerDayB117.put(LocalDate.parse("2021-01-16").
	...
	plusDays(1), 1);
		...
		virusStrainConfigGroup.getOrAddParams(VirusStrain.ALPHA).

	setInfectiousness(1.7);
		virusStrainConfigGroup.getOrAddParams(VirusStrain.ALPHA).

	setFactorSeriouslySick(0.5);
		virusStrainConfigGroup.getOrAddParams(VirusStrain.ALPHA).

	setFactorSeriouslySickVaccinated(0.5);
	...
	Map<LocalDate, Integer> infPerDayMUTB = new HashMap<>();
		...
		infPerDayMUTB.put(LocalDate.parse("2021-06-21").
	...
	//		double deltaInf = 2.7;
	double deltaHos = 0.7;
		virusStrainConfigGroup.getOrAddParams(VirusStrain.DELTA).

	setInfectiousness(params.deltaInf);
		virusStrainConfigGroup.getOrAddParams(VirusStrain.DELTA).

	setFactorSeriouslySick(deltaHos);
		virusStrainConfigGroup.getOrAddParams(VirusStrain.DELTA).

	setFactorSeriouslySickVaccinated(deltaHos);
	...
	Map<Integer, Double> vaccinationCompliance = new HashMap<>();
		for(
	int i = 0;
	i< 5;i++)vaccinationCompliance.put(i,0.0);
		for(
	int i = 5;
	i <=11;i++)vaccinationCompliance.put(i,0.4);
		for(
	int i = 12;
	i <=120;i++)vaccinationCompliance.put(i,1.0);
