...
public class CologneJR implements BatchRun<CologneJR.Params> {
	...
	public Module getBindings(int id, @Nullable Params params) {
		return Modules.override(getBindings(0.0, params)).with(new AbstractModule() {
			...
			protected void configure() {
				...
				if (params != null) {
					mutEscBa5 = params.mutEscBa5;
					mutEscStrainA = params.mutEscStrainA;
	...
	private SnzCologneProductionScenario getBindings(double pHousehold, Params params) {
		return new SnzCologneProductionScenario.Builder()
				...
				.setSebastianUpdate(params == null ? false : Boolean.parseBoolean(params.sebastianUpdate))
...
//		STRAIN_A - aka BA.5
//		double mutAInf = 1.0;
		if (params.strainAInf > 0) {
			double oHos = virusStrainConfigGroup.getParams(VirusStrain.OMICRON_BA2).getFactorSeriouslySick();

			Map<LocalDate, Integer> infPerDayStrainA = new HashMap<>();
			infPerDayStrainA.put(LocalDate.parse("2020-01-01"), 0);
			infPerDayStrainA.put(LocalDate.parse(params.strainADate), 4);
			infPerDayStrainA.put(LocalDate.parse(params.strainADate).plusDays(7), 1);
			episimConfig.setInfections_pers_per_day(VirusStrain.STRAIN_A, infPerDayStrainA);
			virusStrainConfigGroup.getOrAddParams(VirusStrain.STRAIN_A).setInfectiousness(virusStrainConfigGroup.getParams(VirusStrain.OMICRON_BA2).getInfectiousness() * params.ba5Inf * params.strainAInf);
...
//	}
	...
	public static final class Params {
		...
		// BA5
		@StringParameter({"2022-03-27", "2022-04-03", "2022-04-10"})
		public String ba5Date;

		@Parameter({0.0, 0.9,1.0,1.1, 1.2})
		double ba5Inf;

		@Parameter({3.})
		public double mutEscBa5;
		...
		@Parameter({1., 6.})
		public double mutEscStrainA;

		@Parameter({0.0,1.0})
		public double strainAInf;


		// vaccination campaign
		@StringParameter({"2022-09-01"})
