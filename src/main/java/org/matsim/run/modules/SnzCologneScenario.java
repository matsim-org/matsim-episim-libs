...
/**
 * Scenario for Cologne using Senozon data.
 */
public final class SnzCologneScenario extends AbstractModule {
	...
	public Config config() {
		...
		config.plans().setInputFile(INPUT.resolve("cologne_snz_entirePopulation_emptyPlans_withDistricts_25pt_split.xml.gz").toString());

		episimConfig.addInputEventsFile(INPUT.resolve("cologne_snz_episim_events_wt_25pt_split.xml.gz").toString())
				.addDays(DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY, DayOfWeek.THURSDAY, DayOfWeek.FRIDAY);

		episimConfig.addInputEventsFile(INPUT.resolve("cologne_snz_episim_events_sa_25pt_split.xml.gz").toString())
				.addDays(DayOfWeek.SATURDAY);

		episimConfig.addInputEventsFile(INPUT.resolve("cologne_snz_episim_events_so_25pt_split.xml.gz").toString())
...
//			int offset = (int) (ChronoUnit.DAYS.between(episimConfig.getStartDate(), LocalDate.parse("2020-04-01")) + 1);
		...
		restrictions.setInput(INPUT.resolve("KölnSnzData_daily_until20210802.csv"));
		...
		config.controler().setOutputDirectory("output-snz-cologne");
