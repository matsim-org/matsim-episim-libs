 ...
 */
public abstract class LocationBasedAbstractContactModel implements ContactModel {
	public static final String QUARANTINE_HOME = "quarantine_home";
	private static final boolean LOCATION_BASED_RESTRICTIONS_ACTIVE = true;
	...
	Map<String, String> berlinFacilities;

	LocationBasedAbstractContactModel(SplittableRandom rnd, Config config, InfectionModel infectionModel, EpisimReporting reporting) {
		this.rnd = rnd;
		...
		for (ActivityFacility facility : scenario.getActivityFacilities().getFacilities().values()) {
			...
			if (kiez != null) {
				this.berlinFacilities.put(facility.getId().toString(), kiez);
			}
		}
	}
	...
	private boolean activityRelevantForInfectionDynamics(EpisimPerson person, EpisimContainer<?> container, Map<String, Restriction> restrictions, SplittableRandom rnd) {
		...
		if (LOCATION_BASED_RESTRICTIONS_ACTIVE) {
			return actIsRelevantLocationBased(act, restrictions, rnd, person, container);
		}
	...
	private boolean actIsRelevantLocationBased(EpisimPerson.Activity act, Map<String, Restriction> restrictions, SplittableRandom rnd, EpisimPerson person, EpisimContainer<?> container) {
		...
		// Uses district specific remainingFraction, if specified (jr)
		// PERSONS HOME ADDRESS
		//		if (person.getAttributes().getAsMap().containsKey("berlin-neighborhood")) { //district
		//			String district = (String) person.getAttributes().getAttribute("berlin-neighborhood");
