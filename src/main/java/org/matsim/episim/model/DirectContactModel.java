...
public final class DirectContactModel extends AbstractContactModel {
	...
	List<List<EpisimPerson>> groups = new ArrayList<>();
	EpisimPerson singlePerson = null ;
	...
	@Override public void infectionDynamicsVehicle(EpisimPerson personLeavingVehicle, EpisimVehicle vehicle, double now) {
		infectionDynamicsGeneralized(personLeavingVehicle, vehicle, now);
	}
	@Override public void infectionDynamicsFacility(EpisimPerson personLeavingFacility, EpisimFacility facility, double now, String actType) {
	...
	private void notifyEnterContainerGeneralized( EpisimPerson personEnteringContainer, EpisimContainer<?> container, double now ) {
		if ( singlePerson==null ) {
			singlePerson = personEnteringContainer;
		} else {
			groups.add( Arrays.asList( personEnteringContainer, singlePerson) );
			singlePerson = null;
		}
	}
	private List<EpisimPerson> findGroup( EpisimPerson person ) {
		// yy find more efficient data structure?  kai, aug'20
		for( List<EpisimPerson> group : groups ){
	...
	private void infectionDynamicsGeneralized(EpisimPerson personLeavingContainer, EpisimContainer<?> container, double now) {
		...
		List<EpisimPerson> group = findGroup( personLeavingContainer );
		groups.remove( group );
		group.remove( personLeavingContainer );
		singlePerson = group.get( 0 );
		EpisimPerson contactPerson = group.get( 0 );
		group.clear();
...
//		}
	...
	private void removePersonFromGroups( EpisimPerson personLeavingContainer ){
		if ( personLeavingContainer==singlePerson ) {
			singlePerson=null;
		} else {
			List<EpisimPerson> group = findGroup( personLeavingContainer );
