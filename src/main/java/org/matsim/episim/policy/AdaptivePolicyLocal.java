 ...
 */
public class AdaptivePolicyLocal extends ShutdownPolicy {
	...
	/**
	 * Store incidence for each day, per subdistrict
	 */
	private final Map<String,Object2DoubleSortedMap<LocalDate>> cumCasesPerSubdistrict = new HashMap<>();

	/**
	 * Whether activity is currently in lockdown, per subdistrict
	 */
	private final Map<String,Object2BooleanMap<String>> inLockdownSubdistricts = new HashMap<>();

	/**
	 * Constructor from config.
	 */
	public AdaptivePolicyLocal(Config config) {
		super(config);
	...
	/**
	 * Create a config builder for {@link AdaptivePolicyLocal}.
	 */
	...
	public void init(LocalDate start, ImmutableMap<String, Restriction> restrictions) {
		if (initialPolicy != null && !initialPolicy.isEmpty()) {
			...
			for (Map.Entry<String, Restriction> e : restrictions.entrySet()) {
				updateRestrictions(start, initialPolicy, e.getKey(), e.getValue(),null); //TODO
			}
		}
	...
	public void updateRestrictions(EpisimReporting.InfectionReport report, ImmutableMap<String, Restriction> restrictions) {
		throw (new RuntimeException("shouldn't be used for local restriction"));
	}

	public void updateRestrictionsLocal(Map<String,EpisimReporting.InfectionReport> reports, ImmutableMap<String, Restriction> restrictions) {

		for (String subdistrict : reports.keySet()) {
			EpisimReporting.InfectionReport report = reports.get(subdistrict);

			inLockdownSubdistricts.putIfAbsent(subdistrict, new Object2BooleanLinkedOpenHashMap<>());


			LocalDate date = LocalDate.parse(report.date);

			calculateCases(subdistrict,report);
//			cumCasesPerSubdistrict.putIfAbsent(subdistrict, new Object2DoubleAVLTreeMap<>());
			Object2DoubleSortedMap<LocalDate> cases = cumCasesPerSubdistrict.get(subdistrict).tailMap(date.minus(INTERVAL_DAY + 6, ChronoUnit.DAYS));
			...
			for (Map.Entry<String, ConfigValue> e : incidenceTriggers.entrySet()) {
				...
				if (inLockdownSubdistricts.get(subdistrict).getBoolean(act)) {
					if (incidence.values().stream().allMatch(inc -> inc <= trigger.get(0))) {
						updateRestrictions(date, openPolicy, act, restrictions.get(act),subdistrict); // Replace with district
						inLockdownSubdistricts.get(subdistrict).put(act, false);
					}

				} else {
					if (incidence.getDouble(incidence.lastKey()) >= trigger.get(1)) {
						updateRestrictions(date, restrictedPolicy, act, restrictions.get(act),subdistrict); // Replace with district
						inLockdownSubdistricts.get(subdistrict).put(act, true);
					}
				}
			}
		}
	 ...
	 */
	private void calculateCases(String subdistrict,EpisimReporting.InfectionReport report) {
		double cases = report.nShowingSymptomsCumulative * (100_000d / report.nTotal());
		this.cumCasesPerSubdistrict.putIfAbsent(subdistrict, new Object2DoubleAVLTreeMap());
		this.cumCasesPerSubdistrict.get(subdistrict).put(LocalDate.parse(report.date), cases);
	}

	private void updateRestrictions(LocalDate start, Config policy, String act, Restriction restriction, String subdistrict) {
		...
		for (Map.Entry<String, ConfigValue> days : actConfig.root().entrySet()) {
			...
			if (date.isBefore(start)) {
				...
				restrictionForSingleDistrict.put(subdistrict, r.getLocationBasedRf().get(subdistrict));
				Restriction restrictionForDistrict = Restriction.ofLocationBasedRf(restrictionForSingleDistrict);

				restriction.updateLocationBasedRf(restrictionForDistrict);
			}
		}
	}

	/**
	 * Config builder for {@link AdaptivePolicyLocal}.
	 */
	@SuppressWarnings("unchecked")
	public static final class ConfigBuilder extends ShutdownPolicy.ConfigBuilder<Object> {

		/**
		 * Use {@link #AdaptivePolicyLocal #config()}.
		 */
		...
		/**
		 * See {@link AdaptivePolicyLocal#openPolicy}.
		 */
		...
		/**
		 * See {@link AdaptivePolicyLocal#restrictedPolicy}.
		 */
